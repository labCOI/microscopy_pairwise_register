function [Y1, X1, index_ImMatch1, matchedNumb1, pointsPreviousNumb1, pointsNumb1, inlierNumb1, status, error, time, matchedPoints, matchedPointsPrev, pointsPrevious, points, inlierPoints, inlierPointsPrev] = compute_trasnform_north(feature_function, I1, I2, index_matrix, Y_pixel, OvY, M, N, Threshold_metric)
t2 = tic;
...
if feature_function=="detectSIFTFeatures"
    ...
    %pointsPrevious = feval(feature_function,I1, 'ROI',[1 1 N Y_pixel],'MetricThreshold', Threshold_metric);  %detectHarrisFeatures(); %detectBRISKFeatures();  %detectMinEigenFeatures();   %detectMSERFeatures(); %detectMSERFeatures(); %detectORBFeatures(); %detectKAZEFeatures(); %detectSIFTFeatures();
    ...
    %points = feval(feature_function,I2,'ROI',[1 round(M*(1-OvY)) N Y_pixel],'MetricThreshold', Threshold_metric); %detectHarrisFeatures(); %detectBRISKFeatures();  %detectMinEigenFeatures();   %detectMSERFeatures(); %detectMSERFeatures(); %detectORBFeatures(); %detectKAZEFeatures(); %detectSIFTFeatures();
    [features, points] = extractFeatures(I2, points,'Upright',true);
end
