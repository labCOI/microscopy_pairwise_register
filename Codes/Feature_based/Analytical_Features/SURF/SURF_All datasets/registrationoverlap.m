function mainn = registrationoverlap(location, feature_type, nb_horz_tiles, nb_vert_tiles, OverlapY, OverlapX, End, f_type, d_type)
...
mainn.SSIM_north  = NaN(nb_vert_tiles,nb_horz_tiles);
mainn.SSIM_west  = NaN(nb_vert_tiles,nb_horz_tiles);
    ...
    for i = 1:nb_vert_tiles
        ...
        if i > 1
            ...
            [mainn.Ty_north(i,j), mainn.Tx_north(i,j), mainn.index_ImMatch_north(i,j), mainn.matchedNumb_north(i,j),...
                ...
                mainn.inlierPoints_north{i,j}, mainn.inlierPointsPrev_north{i,j}]  = compute_trasnform_north(feature_function,I1, I2, index_matrix(i,j),round(M*OverlapY), OverlapY, M, N, Threshold_metric);
        end
        if j > 1
            % perform pciam west
            if channel == 3
            ...
            [mainn.Ty_west(i,j), mainn.Tx_west(i,j), mainn.index_ImMatch_west(i,j) , mainn.matchedNumb_west(i,j),...
                ...
                mainn.inlierPoints_west{i,j}, mainn.inlierPointsPrev_west{i,j}]= compute_trasnform_west(feature_function,I1, I2, index_matrix(i,j),round(N*OverlapX), OverlapX, M, N, Threshold_metric);
        end
    end
end
    ...
    for i = 1:nb_vert_tiles
        ...
        if i > 1
            ...
            if ~isnan( mainn.index_ImMatch_north(i,j))
                ...
                %mainn.SSIM_north(i,j) = ssim(sub_I1,sub_I2);
                mu1 = mean( sub_I1(:)./max(sub_I1(:)) ); mu2 = mean( sub_I2(:)./max(sub_I2(:)) );
                var1 = var( sub_I1(:)./max(sub_I1(:)) ); var2 = var( sub_I2(:)./max(sub_I2(:)) );
                cov12 = cov( sub_I1(:)./max(sub_I1(:)) , sub_I2(:)./max(sub_I2(:)) );
                mainn.SSIM_north(i,j) = ( (2*mu1*mu2) * (2*cov12(1,2)) )/( ( mu1^2 + mu2^2 ) * ( var1 + var2 ) );
        ...
        if j > 1
            ...
            if ~isnan( mainn.index_ImMatch_west(i,j))
                ...
                %mainn.SSIM_west(i,j) = ssim(sub_I1,sub_I2);
                mu1 = mean( sub_I1(:)./max(sub_I1(:)) ); mu2 = mean( sub_I2(:)./max(sub_I2(:)) );
                var1 = var( sub_I1(:)./max(sub_I1(:)) ); var2 = var( sub_I2(:)./max(sub_I2(:)) );
                cov12 = cov( sub_I1(:)./max(sub_I1(:)) , sub_I2(:)./max(sub_I2(:)) );
                mainn.SSIM_west(i,j) = ( (2*mu1*mu2) * (2*cov12(1,2)) )/( ( mu1^2 + mu2^2 ) * ( var1 + var2 ) );
            end
        end
    end
end
mainn.average_RMSE = mean(cat(2,mean(mainn.RMSE_north(~isnan(mainn.RMSE_north))),mean(mainn.RMSE_west(~isnan(mainn.RMSE_west)))));
mainn.average_SSIM = mean(cat(2,mean(mainn.SSIM_north(~isnan(mainn.SSIM_north))),mean(mainn.SSIM_west(~isnan(mainn.SSIM_west)))));
end