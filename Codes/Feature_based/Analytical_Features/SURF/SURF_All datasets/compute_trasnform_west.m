function [Y2, X2, index_ImMatch2, matchedNumb2, pointsPreviousNumb2, pointsNumb2, inlierNumb2, status,error, time, matchedPoints, matchedPointsPrev, pointsPrevious, points, inlierPoints, inlierPointsPrev] = compute_trasnform_west(feature_function, I1, I2, index_matrix, X_pixel, OvX, M, N, Threshold_metric)
t3  = tic;
...
if feature_function=="detectSIFTFeatures"
    ...
    %pointsPrevious = feval(feature_function,I1, 'ROI',[1 1 X_pixel M],'MetricThreshold', Threshold_metric); %detectHarrisFeatures(); %detectBRISKFeatures();  %detectMinEigenFeatures();   %detectMSERFeatures(); %detectMSERFeatures(); %detectORBFeatures(); %detectKAZEFeatures(); %detectSIFTFeatures();
    ...
    %points = feval(feature_function,I2, 'ROI',[round(N*(1-OvX)) 1 X_pixel M,'MetricThreshold', Threshold_metric]); %detectHarrisFeatures(); %detectBRISKFeatures();  %detectMinEigenFeatures();   %detectMSERFeatures(); %detectMSERFeatures(); %detectORBFeatures(); %detectKAZEFeatures(); %detectSIFTFeatures();
    [features, points] = extractFeatures(I2, points,'Upright',true);
end
